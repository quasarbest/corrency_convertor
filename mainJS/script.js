...
axios.get("https://www.cbr-xml-daily.ru/daily_json.js")
    .then((res) => {
        ...
        USDviuw.textContent = (valute.USD).toFixed(2)
        KZTviuw.textContent = (100 / valute.KZT).toFixed(2)
        
    });
    
kzInput.addEventListener('change', () => {
    kzInRub.textContent = ((kzInput.value / valute.KZT) * 100).toFixed(2) + "  тенге"
    console.log(valute.KZT);
});

usdInput.addEventListener('change', () => {
    usdInRub.textContent = (usdInput.value * (valute.KZT / 100)).toFixed(2) + "  рублей"
...
function clear() {
    document.querySelector('.clear_value').addEventListener('click', function () {
        valueTabl.value = ''
    });
};
clear();